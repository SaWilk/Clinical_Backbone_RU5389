...
out_path          <- file.path(project_data_path, "raw_data")
derivatives_path  <- file.path(project_data_path, "derivatives")
...
# Keep exported/preprocessed data and derived/scored data strictly separate.
# All output that previously lived directly below 01_project_data is now written
# below 01_project_data/raw_data. The cognitive scoring pipeline mirrors this
# structure below 01_project_data/derivatives.
dir.create(out_path, recursive = TRUE, showWarnings = FALSE)
dir.create(derivatives_path, recursive = TRUE, showWarnings = FALSE)

# One-time migration of the former output layout. Only known pipeline output
# directories are moved; sync metadata and unrelated files remain untouched.
legacy_output_names <- c(
  paste0(2:9, "_backbone"),
  "all_projects_backbone", "discarded", "logs", "old_data", "out", "pilots"
)
for (legacy_name in legacy_output_names) {
  legacy_path <- file.path(project_data_path, legacy_name)
  migrated_path <- file.path(out_path, legacy_name)
  if (!file.exists(legacy_path) && !dir.exists(legacy_path)) next
  if (file.exists(migrated_path) || dir.exists(migrated_path)) {
    stop(
      "Cannot migrate legacy output because source and destination both exist: ",
      legacy_path, " -> ", migrated_path,
      ". Resolve this conflict manually before rerunning."
    )
  }
  if (!file.rename(legacy_path, migrated_path)) {
    stop("Could not migrate legacy output: ", legacy_path, " -> ", migrated_path)
  }
  message("Migrated legacy output: ", legacy_path, " -> ", migrated_path)
}
...
log_copy <- copy_psytool_files(
  ...
  cogtest_out_path  = out_path,          # "01_project_data/raw_data"
...
# Copy P9 adults pilot experiment files under 01_project_data/raw_data/9_backbone/pilot_data/experiment_data/...
copy_psytool_files(
