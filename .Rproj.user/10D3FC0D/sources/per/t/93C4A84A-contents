...
#     B) Empirical timeline + per-project linear regression with 95% CI and horizon = 2x current duration
...
datestamp_last <- format(last_date, "%Y-%m-%d")
...
# split: p6 children MUST be EXACT same green as p6
color_map_split <- c(
  color_map,
  "p8 children"="#000000",
  "p8 adults"  ="#A6761D",
...
# ---- Regression (linear) + 95% Prediction Interval ----
fit_project_lm <- function(df_grp){
  ...
  s <- summary(mod)
  list(model = mod, sigma = s$sigma, df = stats::df.residual(mod))
}
predict_lm_pi <- function(fit, date_seq){
  newd <- tibble::tibble(date = date_seq, tnum = as.numeric(date_seq))
  prd  <- stats::predict(fit$model, newdata = newd, se.fit = TRUE)
  tval <- stats::qt(0.975, df = fit$df)
  se_mean <- as.numeric(prd$se.fit)
  sigma   <- fit$sigma
  se_pred <- sqrt(se_mean^2 + sigma^2)
  tibble::tibble(
    date = date_seq,
    fit  = pmax(0, as.numeric(prd$fit)),
    lwr  = pmax(0, as.numeric(prd$fit - tval * se_pred)),
    upr  = pmin(Inf, as.numeric(prd$fit + tval * se_pred))
  )
}
...
plot_timeline <- function(df, layout = c("vanilla","split"), with_pred = FALSE,
  ...
  if (has_pred) {
    ...
    pred_df <- purrr::map_dfr(pred_df, function(grp){
      ...
      preds <- predict_lm_pi(fitinfo, dseq) %>%
  ...
  perc_sub_pred <- paste0(perc_sub_emp, " • Shaded: 95% PI = \u0177\u0302 \u00B1 t\u2080.\u2089\u2087\u2085,df \u221A(SE_fit\u00B2 + \u03C3\u00B2)")
