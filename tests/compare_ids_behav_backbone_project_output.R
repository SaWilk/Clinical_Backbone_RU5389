...
# Outputs project information next to each ID
# ============================================================================
...
path_questionnaire <- "K:/Wilken_Arbeitsordner/Clinical_Backbone_RU5389/03_analysis_input/adults_adolescents_complete_items_lt030_enriched.xlsx"
...
# -------------------------------------------------------------------------
# Helper: collapse project information for duplicated IDs
# -------------------------------------------------------------------------
# Falls eine ID mehrfach vorkommt und dabei unterschiedliche project-Werte hat,
# werden diese Werte zusammengeführt, statt stillschweigend nur den ersten zu nehmen.

collapse_project <- function(x) {
  x <- unique(x[!is.na(x)])
  if (length(x) == 0) {
...
if (!"project" %in% names(questionnaire)) {
  stop("Column 'project' was not found in questionnaire data.")
}

ids1_raw <- questionnaire$vp_id
project1_raw <- questionnaire$project
...
if (!"project" %in% names(behavioral)) {
  stop("Column 'project' was not found in behavioral data.")
}

ids2_raw <- behavioral$ID
project2_raw <- behavioral$project
...
project1 <- as.character(project1_raw)
project2 <- as.character(project2_raw)
...
questionnaire_id_project <- tibble::tibble(
  vp_id = ids1,
  project = project1
) %>%
  filter(!is.na(vp_id)) %>%
  group_by(vp_id) %>%
  summarise(
    project = collapse_project(project),
    .groups = "drop"
  )

behavioral_id_project <- tibble::tibble(
  ID = ids2,
  project = project2
) %>%
  filter(!is.na(ID)) %>%
...
only_in_questionnaire_tbl <- tibble::tibble(
  vp_id = sort(only_in_questionnaire)
) %>%
  left_join(questionnaire_id_project, by = "vp_id")

only_in_behavioral_tbl <- tibble::tibble(
  ID = sort(only_in_behavioral)
) %>%
  left_join(behavioral_id_project, by = "ID")

in_both_tbl <- tibble::tibble(
  id = sort(in_both)
) %>%
  left_join(
    questionnaire_id_project %>%
      rename(
        id = vp_id,
        project_questionnaire = project
      ),
    by = "id"
  ) %>%
  left_join(
    behavioral_id_project %>%
      rename(
        id = ID,
        project_behavioral = project
      ),
    by = "id"
  ) %>%
  mutate(
    project_same = project_questionnaire == project_behavioral
  )
